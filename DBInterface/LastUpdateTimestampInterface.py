...
class LastUpdateTimestampInterface():
    ...
    def set_log_file_last_update_timestamp(self,type_id,timestamp):
        if isinstance(type_id,str):
            ...
            query.filter(LastUpdateTimestampTable.type_id == type_id).update({LastUpdateTimestampTable.last_file_update_timestamp:str(timestamp)})
            self.write_to_db()
    ...
    def set_record_last_update_timestamp(self,type_id,timestamp):
        if isinstance(type_id,str):
            self.touch_new_record(type_id=type_id)
            query = self.session.query(LastUpdateTimestampTable)
            query.filter(LastUpdateTimestampTable.type_id == type_id).update({LastUpdateTimestampTable.last_record_update_timestamp:str(timestamp)})
            self.write_to_db()
